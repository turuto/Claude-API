// None of the prompts below are the prompt being tested — that one lives in index.js.
// These are the behind-the-scenes instructions Claude uses to invent test scenarios,
// build full test cases from them, and grade the outputs your prompt produces.

export function buildIdeasPrompt(taskDescription, promptInputsSpec, numCases) {
    const promptInputsList = Object.entries(promptInputsSpec)
        .map(([key, value]) => `"${key}": str // ${value}`)
        .join(',\n');

    return `Generate ${numCases} unique, diverse ideas for testing a prompt that accomplishes this task:
...
export function buildTestCasePrompt(taskDescription, idea, promptInputsSpec) {
    const allowedKeys = Object.keys(promptInputsSpec)
        .map((key) => `"${key}"`)
        .join(', ');

    const exampleInputs = Object.entries(promptInputsSpec)
        .map(([key, value]) => `"${key}": "EXAMPLE_VALUE", // ${value}`)
        .join('\n');

    return `Generate a single detailed test case for a prompt evaluation based on:
...
export function buildEvalPrompt(testCase, output, extraCriteria) {
...
`
    ...
    return `Your task is to evaluate the following AI-generated solution with EXTREME RIGOR.
...
<criteria>
${testCase.solution_criteria.join('\n')}
</criteria>
